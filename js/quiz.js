...
function drawQuiz(){
  ...
  if (qdone){
    ...
    box.innerHTML = `<div class="q-result"><p class="eyebrow">Resultado</p>
      <div class="big">${qscore} / ${total}</div>
      <p class="best">Mejor marca en este navegador: ${Math.max(prevBest, qscore)} / ${total}</p>
      <div style="margin-top:20px"><button class="btn" id="qretry">Repetir</button></div></div>`;
  ...
  box.innerHTML = `<div class="q-top"><span>Pregunta ${qpos + 1} de ${quiz.items.length}</span><span class="score">Aciertos: ${qscore}</span></div>
    <div class="q-card">
      ...
      <div class="q-foot"><button class="btn hide" id="qnext">${qpos === quiz.items.length - 1 ? "Ver resultado" : "Siguiente →"}</button></div>
    </div>`;
