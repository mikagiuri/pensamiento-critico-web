...
const QUIZ_SUBJECTS = { fil: "Filosofía 1.º", hf: "Historia de la Filosofía", ipc: "Pensamiento crítico" };
const QUIZ_BLOCKS = { A: "Bloque A · Antigua", B: "Bloque B · Medieval-Moderna", C: "Bloque C · Contemporánea" };
...
function drawQuiz(){
  ...
  if (qdone){
    ...
    if (!quizReviewing){
      ...
      bestLine = `<p class="best">Mejor marca en este navegador: ${Math.max(prevBest, qscore)} / ${total}</p>`;
    }
    const reviewBtn = quizFailed.length
      ? `<button class="btn" id="qreview">Repasar las que fallé (${quizFailed.length})</button>` : "";
    box.innerHTML = `<div class="q-result"><p class="eyebrow">${quizReviewing ? "Repaso" : "Resultado"}</p>
      <div class="big">${qscore} / ${total}</div>
      ${bestLine}
      <div style="margin-top:20px;display:flex;gap:10px;flex-wrap:wrap;justify-content:center"><button class="btn" id="qretry">Repetir</button>${reviewBtn}</div></div>`;
  ...
  box.innerHTML = `<div class="q-top"><span>Pregunta ${qpos + 1} de ${total}</span><span class="score">Aciertos: ${qscore}</span></div>
    <div class="q-card">
      ...
      <div class="q-foot"><button class="btn hide" id="qnext">${qpos === total - 1 ? "Ver resultado" : "Siguiente →"}</button></div>
    </div>`;
