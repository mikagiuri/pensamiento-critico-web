...
const UNIDAD_BLOCKS = { A: "Bloque A · Antigua", B: "Bloque B · Medieval-Moderna", C: "Bloque C · Contemporánea" };
...
function renderUnidadFilter(){
  ...
  const btns = ["all", "A", "B", "C"].map(b =>
    ...
    (b === "all" ? "Todos los bloques" : UNIDAD_BLOCKS[b]) + '</button>'
  ).join("");
...
function renderUnidadBody(){
  ...
  const media = MEDIA_TEMAS.filter(e => e.t === t).map(e => {
    ...
    return (v || i) ? '<ul class="mlist">' + v + i + '</ul>' : '<p class="muted">Sin recursos multimedia listados</p>';
  }).join("") || '<p class="muted">Sin recursos multimedia listados</p>';
  ...
  const quizBtn = quizKey && QUIZZES[quizKey]
    ...
    : '<p class="muted">Test: por completar</p>';
  ...
  if (toc){
    ...
    toc.innerHTML = '<div class="toc-title">En esta unidad</div><ol>' +
