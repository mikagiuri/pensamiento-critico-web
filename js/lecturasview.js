...
const LECTURA_BLOCKS = { A: "Bloque A · Antigua", B: "Bloque B · Medieval-Moderna", C: "Bloque C · Contemporánea" };
...
function renderLecturaFilter(){
  ...
  const blockBtns = ["all", "A", "B", "C"].map(b =>
    ...
    (b === "all" ? "Todos los bloques" : LECTURA_BLOCKS[b]) + '</button>'
  ).join("");
  box.innerHTML = '<div class="fgroup"><span class="flabel">Bloque</span>' + blockBtns + '</div>';
...
function loadLectura(k){
  ...
  toc.innerHTML = '<div class="toc-title">En esta lectura</div><ol>' +
    hs.map((h, i) => '<li><a href="#lh-' + i + '">' + h.textContent + '</a></li>').join("") + '</ol>';
}

renderLecturaFilter();
loadLectura(lecturaKey);
